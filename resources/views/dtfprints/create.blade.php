...
@section('title', 'Texon - Registrar impressão DTF')

@section('content')
    <h1>Registrar impressão DTF</h1>
    <div class="dropdown">
    ...
    <br>
    <form method="POST" action="{{ route('dtfprints.store') }}">
        @csrf
        <div class="row mb-3">
            <div class="col-md-6">
            ...
            <div class="col-md-12">
                ...
                <select name="status" id="status" class="form-select">
                    @foreach ($statuses as $s)
                        <option class="text-uppercase" value="{{ $s }}">{{ ucfirst($s) }}</option>
                    @endforeach
                </select>
            </div>
        </div>
